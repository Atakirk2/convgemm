...
//void Pack_A(double *A, unsigned int lda, double *A_pack, unsigned int m, unsigned int k, int * n_threads)
void Pack_A(double *A, unsigned int lda, double *A_pack, unsigned int m, unsigned int k)
{
	double *A_pack_local;
...
//OLD definition, mantained for documentation purposes
//void Pack_B(double *B, unsigned int ldb, double *B_pack, unsigned int k, unsigned int n, int * n_threads)
void Pack_B(double *B, unsigned int ldb, double *B_pack, unsigned int k, unsigned int n)
...
void dgemm_conv(unsigned int m, unsigned int n, unsigned int k,
    ...
    posix_memalign(&Ac_pack, 4096, 12*BLOCK_MC*BLOCK_KC*sizeof(double));
	...
	for (unsigned int jc=0; jc<n; jc+=BLOCK_NC) {
		...
		for (unsigned int pc=0; pc<k; pc+=BLOCK_KC) {
			...
			for (unsigned int ic=0; ic<m; ic+=BLOCK_MC) {
				...
				Ac=&A[ic+pc*lda];
				Pack_A(Ac,lda,(double*)Ac_pack_local,m_alg,k_alg); //PACK A
